...
def installGrub(mounts, disk):
    ...
    grubconf += "   kernel /xen-%s.gz lowmem_emergency_pool=16M dom0_mem=196M\n" % version.xen_version
    ...
    grubconf += "   root (%s,%s)\n" % (getGrUBDevice(disk, mounts), getBootPartNumber(disk) - 1)
    grubconf += "   kernel /xen-%s.gz com1=115200,8n1 console=com1,tty lowmem_emergency_pool=16M dom0_mem=196M\n" % version.xen_version
    ...
    grubconf += "   root (%s,%s)\n" % (getGrUBDevice(disk, mounts), getBootPartNumber(disk) - 1)
    grubconf += "   kernel /xen-%s.gz noacpi nousb nosmp noreboot com1=115200,8n1 console=com1,tty dom0_mem=196M\n" % version.xen_version
    grubconf += "   module /vmlinuz-%s ramdisk_size=75000 root=/dev/ram0 ro console=tty0 console=ttyS0,115200n8\n" % version.kernel_version
    grubconf += "   module /%s-%s.img\n" % (version.dom0_name, version.dom0_version)
