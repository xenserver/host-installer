...
def setupSWRAIDDevice(disk_label_suffix, physical_disks, guest_disks):
    ...
    if any(disk in physical_disks for disk in guest_disks):
        guest_disks = list(filter(lambda disk: disk not in physical_disks, guest_disks))
...
def umountVolumes(disk, mounts, cleanup, force=False):
    ...
    if diskutil.is_raid(disk):
        rc = util.runCmd2(['mdadm', '--stop', disk])
        if rc != 0:
            raise RuntimeError("Failed to stop RAID device: '%s'" % disk)
