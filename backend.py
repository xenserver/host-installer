...
def configureNetworking(mounts, admin_iface, admin_bridge, admin_config, hn_conf, ns_conf, nethw, preserve_settings, iscsi_iface):
    ...
    if admin_iface != iscsi_iface:
        # This interface should be managed by xapi rather than brought up on boot, and should
        # belong to a bridge so that vifs can be attached to the mgmt network.
        ...
        # If this interface is needed for an iscsi root disk then it should be brought up at boot
        # and never reconfigured by xapi or added to a bridge as this would cause the root filesystem
        # to become inaccessable.
        print >>sysconf_admin_iface_fd, "ONBOOT=yes"
        if not admin_config.isStatic():
            print >>sysconf_admin_iface_fd, "BOOTPROTO=dhcp"
        else:
            print >>sysconf_admin_iface_fd, "BOOTPROTO=none"
            print >>sysconf_admin_iface_fd, "NETMASK=%s" % admin_config.netmask
            print >>sysconf_admin_iface_fd, "IPADDR=%s" % admin_config.ipaddr
            if admin_config.gateway:
                print >>sysconf_admin_iface_fd, "GATEWAY=%s" % admin_config.gateway
            if manual_nameservers:
                print >>sysconf_admin_iface_fd, "PEERDNS=yes"
                for i in range(len(nameservers)):
                    print >>sysconf_admin_iface_fd, "DNS%d=%s" % (i+1, nameservers[i])
            if domain:
                print >>sysconf_admin_iface_fd, "DOMAIN=%s" % domain
