...
def getFinalisationSequence(ans):
    seq = [
        ...
        Task(installBootLoader, A(ans, 'mounts', 'primary-disk',
                                  ...
                                  'serial-console', 'boot-serial', 'host-config', 'fcoe-interfaces', 'swraid'), []),
    ...
    if ans['install-type'] == INSTALL_TYPE_FRESH and ans['swraid']:
        seq.append(Task(completeSWRAID, A(ans, 'mounts', 'primary-disk', 'physical-disks', 'write-boot-entry', 'install-type', 'branding', 'boot-partnum', 'primary-partnum', 'logs-partnum', 'target-boot-mode'), ['mounts']))
...
def installBootLoader(mounts, disk, boot_partnum, primary_partnum, target_boot_mode, branding,
                      ...
                      boot_serial=None, host_config=None, fcoe_interface=None, swraid=False):
...
def completeSWRAID(mounts, primary_disk, physical_disks, write_boot_entry, install_type, branding, boot_partnum, primary_partnum, logs_partnum, target_boot_mode, remount=True):
    ...
    rc = util.runCmd2(['mdadm', '--stop', primary_disk])
    if rc != 0:
        raise RuntimeError("Failed to stop SWRAID device: '%s'" % primary_disk)

    if write_boot_entry:
        for disk in physical_disks:
            rc, err = util.runCmd2(["/usr/sbin/efibootmgr", "-c",
                                    "-L", branding['product-brand'], "-l", '\\' + efi.replace('/', '\\'),
                                    "-d", disk, "-p", str(boot_partnum)], with_stderr=True)
            check_efibootmgr_err(rc, err, install_type, "Failed to add new efi boot entry")
