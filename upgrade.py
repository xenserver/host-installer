...
class ThirdGenUpgrader(Upgrader):
    ...
    def buildRestoreList(self, src_base):
        ...
        # SNMP service config
        # From XS 8.4 SNMP feature is supported, and new file /etc/snmp/snmp.xs.conf is added into dom0, 
        # the file can be treated as the feature flag. Host installer must not restore old config files
        # prior to XS 8.4 becasuse they are incompatible with xapi snmp plugin.
