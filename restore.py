...
def restoreFromBackup(backup, progress = lambda x: ()):
    ...
    backup_version = str(backup.version)
    backup_version = backup_version[0:3]
    current_product_list = product.findXenSourceProducts()
    current_product = current_product_list[0]
    current_version = str(current_product.version)
    ...
    if logs_partition and current_version >= '6.6' and backup_version < '6.6': # From 7.x (new layout) to 6.x
        restoreWithRepart(backup,progress)
...
def restoreWithRepart(backup, progress):
    ...
    try:
        ...
        xelogging.log("Mounting restore...")
        dest_fs = util.TempMount(restore_partition, 'restore-dest-')
        xelogging.log("Restore mounted")
        try:
...
def restoreWithoutRepart(backup, progress):
