...
def idFromPartition(partition):
    ...
    if m and 'ID_WWN' in props:
        wwn_link = next((x for x in links if x.startswith(
                         '/dev/disk/by-id/nvme-' + props['ID_WWN'])), None)
        if wwn_link:
            return wwn_link
...
def probeDisk(device):
    ...
    if len(srs) > 1:
        logger.info('Probe of %s found multiple SRs: %s' % (device, srs))
        raise Exception('Cannot handle multiple SRs on a device: %s' % (device,))
    elif srs:
        disk.storage = srs[0]
